...
public:
    ...
    void render(const Rigidbody& rigidbody) {
        ...
        glVertex2f(static_cast<float>(position(0)), static_cast<float>(position(1))); // Center of the circle
        for (int i = 0; i <= numSegments; ++i) {
            float angle = 2.0f * M_PI * i / numSegments;
            float x = radius * cos(angle);
            float y = radius * sin(angle);
            glVertex2f(static_cast<float>(position(0)) + x, static_cast<float>(position(1)) + y);
        }
        glEnd();
